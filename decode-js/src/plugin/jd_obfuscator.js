...
function decodeGlobal(ast) {
  ...
  function find_ob_sort_func(path) {
    function get_ob_sort(path) {
      ...
      path.stop() //停止继续traverse
      path.remove() //代码中删除掉该移位函数
    }
  ...
  function find_ob_sort_list_by_name(path) {
    ...
    if (path.node.id.name == ob_string_func_name) {
      console.log(ob_string_func_name)
  ...
  console.log("ob_string_func_name->", ob_string_func_name)
  if (!ob_string_func_name) {
  ...
  console.log("ob_func_str->", ob_func_str)

  let all_collect_codes = []
  all_collect_codes = all_collect_codes.concat(ob_func_str)
  virtualGlobalEval(ob_func_str.join(';'))  //eval有上下文的，eval后的代码，任然存在于vm2的上下文中哦
  // let tttt = virtualGlobalEval('Zb()')
  // console.log(tttt)
  // 循环删除混淆函数
  let call_dict = {}
  // let exist_names = []
  let collect_codes = []
  let collect_names = ob_dec_name

  let all_de_funs = ob_dec_name
  ...
  function do_collect_func(path) {
    ...
    if (
      ...
      if (all_de_funs.indexOf(call_func) === -1) {
        //外层函数，没有call解密函数，这样的函数就不是壳函数
        return
      }
      let name = path.node.id.name
      let t = generator(path.node, {minified: true}).code
      if (all_de_funs.indexOf(name) === -1 && all_collect_codes.indexOf(t) === -1) {
        all_de_funs.push(name)
        // collect_codes.push(t)
        collect_names.push(name)
        all_collect_codes.push(t)
        return
      }
      if (all_de_funs.indexOf(name) !== -1 && all_collect_codes.indexOf(t) === -1) {
        console.log(`info: 同名函数 ${name}`)
        // 此处在更深层级定义了同名的解密函数，处理：
        // 1.先尝试重命名一下,在父级作用域将函数改名
        let rename_time = new Date().getTime()
        path.parentPath.scope.rename(name, `${name}_${rename_time}`)
        let t = generator(path.node, {minified: true}).code
        // collect_codes.push(t)
  ...
  function replaceWithInitValue(path) {
    // console.log(`curPath -> ${path + ""}`)
    if (!path.node.init) {
      return
    }

    let valueName = path.node.id.name
    let initValue = path.node.init.value
    let initValueType = path.node.init.type
    // let bd = path.scope.getBinding(valueName)
    if (initValueType === "NumericLiteral" && path.scope.getBinding(valueName).referencePaths.length) {
      // console.log(`curPath -> ${path+""}`)
      for (let targetNode of path.scope.getBinding(valueName).referencePaths) {
        if ((targetNode.parentPath + "").indexOf("++") !== -1) {
          return;
        }
        let old = targetNode.parentPath + ""
        targetNode.replaceWith(t[initValueType](initValue))
        console.log("【init replace】 -> ", old + ` ==(${valueName})==> `, targetNode.parentPath + "")
      }

    } else if (
      path.node.init.properties &&
      path.node.init.properties[0] &&
      path.node.init.properties[0].value &&
      t.isObjectExpression(path.node.init) &&
      t.isNumericLiteral(path.node.init.properties[0].value)) {
      let tmpObjxInit
      ...
      // console.log(`${valueName}:${initValue}`)
    }
  }
  ...
  function replaceSequenceExpressionWithConst(path) {
    ...
    // console.log(path+"","==>",constValue)
    // path.replaceWith(t.stringLiteral(constValue))
  }
  ...
  function do_collect_var(path) {
    ...
    if (right?.type === 'Identifier' && all_de_funs.indexOf(right.name) !== -1) {
      let name = left.name
  ...
  //替换字符串解密，对于京东的解密，和一般的字符串解密不一样，需要先把参数替换为常量
  //比如  rt(n, e, o)
  traverse(ast, {VariableDeclarator: replaceWithInitValue})
  ...
  function do_replace(path) {
    ...
    if (Object.prototype.hasOwnProperty.call(call_dict, old_call)) {
      console.log("[replace]", old_call, "==>", call_dict[old_call])
      path.replaceWith(t.StringLiteral(call_dict[old_call]))
    }
  }

  console.log(all_de_funs, final_codes)
  //运行收集到的代码
  virtualGlobalEval(final_codes.join(';'))
...
const deleteObfuscatorCode = {
  ...
  ExpressionStatement(path) {
    ...
    args = Object.hasOwn(callee, "arguments") && Object.getOwnPropertyDescriptor(callee, "arguments")
    if (typeof args !== "object") {
      console.log("can't get callee's arguments!")
      return;
    }
    if (
      args.length == 2 &&
